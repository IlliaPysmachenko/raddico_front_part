...
const StudiesBlock = () => {
    const {studies, totalStudiesCount, totalImagesCount, studyTitles} = useAppSelector(state => state.study)

    const additionalInfo = studies && (<div className={style.additionalInfo}>
        <span>Total studies count: {totalStudiesCount}; </span>
        <span>Total images count: {totalImagesCount}; </span>
    </div>)
    const studiesTable = studies.map((item) => {
        return <React.Fragment key={item.study_iuid}>
    ...
    return (
        <>
            ...
            {studies && (<div className={`${style.study_container}`}>
                {studyTitles.map((item) => (<div key={item.id} className={style.study_header}>{item.title}</div>))}
                {studiesTable}
